...
sdl2_status_redraw(struct SDL2Window *win)
{
    ...
    background = sdl2_text_bg(iflags.wcolors[wcolor_status].fg,
    ...
    if (iflags.wc_align_status == ALIGN_LEFT ||
        ...
        for (i = 0; i < SIZE(conditions); ++i) {
            if ((data->conditions & conditions[i].mask) != 0) {
                const char *str = conditions[i].text[0];
                unsigned j;
                color = NO_COLOR;
                for (j = 0; j < CLR_MAX; ++j) {
                    if (data->colormasks[j] & conditions[i].mask) {
                        color = j;
                        break;
                    }
                }
        ...
        for (i = 0; i < SIZE(conditions); ++i) {
            if ((data->conditions & conditions[i].mask) != 0) {
                ...
                items[num_items].mixed = FALSE;
                ++num_items;
                items[num_items].str = conditions[i].text[0];
                items[num_items].color = NO_COLOR;
                items[num_items].mixed = FALSE;
                for (j = 0; j < CLR_MAX; ++j) {
                    if (data->colormasks[j] & conditions[i].mask) {
...
{
    const char *fgcolor = iflags.wcolors[wcolor_status].fg;
    const char *bgcolor = iflags.wcolors[wcolor_status].bg;
    int attr = color >> 8;
    color &= 0xFF;

...
static void sdl2_display_nhwindow(winid, boolean);
...
static void sdl2_display_file(const char *, boolean);
static void sdl2_start_menu(winid, unsigned long);
static void sdl2_add_menu(winid, const glyph_info *, const anything *, char,
...
static void sdl2_update_inventory(int perm);
...
#endif
static void sdl2_print_glyph(winid, coordxy, coordxy, const glyph_info *,
                             const glyph_info *);
static void sdl2_raw_print(const char *str);
static void sdl2_raw_print_bold(const char *str);
static int  sdl2_nhgetch(void);
static int  sdl2_nh_poskey(coordxy *x, coordxy *y, int *mod);
...
static win_request_info *sdl2_ctrl_nhwindow(winid id, int request,
...
static void sdl2_window_startMenu(struct SDL2Window *win, unsigned long mbehavior);
static void sdl2_window_addMenu(struct SDL2Window *win, const glyph_info *glyph,
...
static void sdl2_window_printGlyph(struct SDL2Window *win, coordxy x, coordxy y,
...
static void sdl2_putmsghistory(const char *str, boolean is_restoring);
static win_request_info *sdl2_window_ctrlNHWindow(struct SDL2Window *win,
        int request, win_request_info *wri);

static SDL_Color color_from_string(const char *color, const char *dcolor,
        unsigned alpha);

static void
sdl2_init_nhwindows(int *argc UNUSED, char **argv UNUSED)
{
...
/*ARGSUSED*/
static Uint32
timer_callback(Uint32 interval, genericptr_t param UNUSED)
{
...
sdl2_askname(void)
{
    ...
    sdl2_getline_run(window, "What is your name?", svp.plname, SIZE(svp.plname));
...
/*ARGSUSED*/
static void
sdl2_suspend_nhwindows(const char *str UNUSED)
{
    /* Place holder */
}
...
static void
sdl2_display_nhwindow(winid id, boolean blocking)
{
    struct SDL2Window *window = find_window(id);
...
static void
sdl2_display_file(const char *filename, boolean complain)
{
    dlb *file;
...
static void
sdl2_start_menu(winid id, unsigned long mbehavior)
{
    struct SDL2Window *window = find_window(id);
    if (window != NULL) {
        sdl2_window_startMenu(window, mbehavior);
    }
}

static void
sdl2_add_menu(winid id, const glyph_info *glyph, const anything *identifier,
              char ch, char gch, int attr, int clr, const char *str,
              unsigned itemflags)
{
    struct SDL2Window *window = find_window(id);
    if (window != NULL) {
        sdl2_window_addMenu(window, glyph, identifier, ch, gch, attr, clr, str,
                            itemflags);
    }
}
...
static void
sdl2_update_inventory(int perm UNUSED)
{
    /* Place holder */
}
...
static void
sdl2_print_glyph(winid id, coordxy x, coordxy y, const glyph_info *glyph,
...
static int
sdl2_nh_poskey(coordxy *x, coordxy *y, int *mod)
{
    Uint32 ch;
...
static char
sdl2_yn_function(const char *ques, const char *choices, char def)
{
...
/*ARGSUSED*/
static void
sdl2_number_pad(int state UNUSED)
{
    /* Place holder */
}
...
sdl2_ctrl_nhwindow(winid id, int request, win_request_info *wri)
{
    struct SDL2Window *window = find_window(id);
    if (window != NULL) {
        return sdl2_window_ctrlNHWindow(window, request, wri);
    } else {
        return NULL;
    }
}
                         ...
                         SDL_Color fg, SDL_Color bg)
{
    ...
    Sprintf(esc, "\\G%04X", svc.context.rndencode);
    ...
    while (*p1 != '\0') {
        ...
        if (SYMHANDLING(H_UTF8) && gmap->u && gmap->u->utf8str) {
            ch32 = gmap->u->utf32ch;
        } else
#endif
        {
            ch = gs.showsyms[gmap->sym.symidx];
            ch32 = sdl2_chr_convert(ch);
        }
        if (gmap->customcolor != 0) {
            uint32 color = gmap->customcolor;
            oc.r = (Uint8)(color >> 16);
            oc.g = (Uint8)(color >>  8);
            oc.b = (Uint8)(color >>  0);
            oc.a = 255;
        } else {
            oc = sdl2_colors[gmap->sym.color];
        }
        txt_rect = sdl2_window_renderCharBG(win, ch32,
                x + all_rect.w, y, oc, bg);
...
sdl2_chr_convert(Uint32 ch)
{
    ...
    if (SYMHANDLING(H_IBM)) {
        return cp437_table[(unsigned char)ch];
...
static void
sdl2_window_startMenu(struct SDL2Window *win, unsigned long mbehavior)
{
    if (win->methods->startMenu != NULL) {
        win->methods->startMenu(win, mbehavior);
    }
}

static void
sdl2_window_addMenu(struct SDL2Window *win, const glyph_info *glyph,
                    const anything *identifier, char ch, char gch, int attr,
                    int clr, const char *str, unsigned itemflags)
{
    if (win->methods->addMenu != NULL) {
        win->methods->addMenu(win, glyph, identifier, ch, gch, attr, clr, str,
                              itemflags);
    }
}
...
static void
sdl2_window_printGlyph(struct SDL2Window *win, coordxy x, coordxy y,
                       const glyph_info *glyph, const glyph_info *bkglyph)
{
...
static void
sdl2_putmsghistory(const char *str, boolean is_restoring)
{
...
static win_request_info *
sdl2_window_ctrlNHWindow(struct SDL2Window *win, int request,
                         win_request_info *wri)
{
    if (win->methods->ctrlNHWindow != NULL) {
        return win->methods->ctrlNHWindow(win, request, wri);
    } else {
        return NULL;
    }
}
...
Uint32
sdl2_get_key(boolean cmd, coordxy *x, coordxy *y, int *mod)
{
    Uint32 ch;
    boolean shift;
    SDL_Event event;
...
static SDL_Color
color_from_string(const char *color, const char *dcolor, unsigned alpha)
{
    SDL_Color rgb;
...
struct window_procs sdl2_procs = {
    ...
    { TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE,
      TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE, TRUE },
    sdl2_init_nhwindows, //  void (*win_init_nhwindows)(int *, char **);
    sdl2_player_selection, //  void (*win_player_selection)(void);
    sdl2_askname, //  void (*win_askname)(void);
    sdl2_get_nh_event, //  void (*win_get_nh_event)(void);
    sdl2_exit_nhwindows, //  void (*win_exit_nhwindows)(const char *);
    sdl2_suspend_nhwindows, //  void (*win_suspend_nhwindows)(const char *);
    sdl2_resume_nhwindows, //  void (*win_resume_nhwindows)(void);
    sdl2_create_nhwindow, //  winid (*win_create_nhwindow)(int);
    sdl2_clear_nhwindow, //  void (*win_clear_nhwindow)(winid);
    sdl2_display_nhwindow, //  void (*win_display_nhwindow)(winid, boolean);
    sdl2_destroy_nhwindow, //  void (*win_destroy_nhwindow)(winid);
    sdl2_curs, //  void (*win_curs)(winid, int, int);
    sdl2_putstr, //  void (*win_putstr)(winid, int, const char *);
    sdl2_putmixed, //  void (*win_putmixed)(winid, int, const char *);
    sdl2_display_file, //  void (*win_display_file)(const char *, boolean);
    sdl2_start_menu, //  void (*win_start_menu)(winid, unsigned long);
    sdl2_add_menu, //  void (*win_add_menu)(winid, const glyph_info *, const ANY_P *, char, char, int, int, const char *, unsigned int);
    sdl2_end_menu, //  void (*win_end_menu)(winid, const char *);
    sdl2_select_menu, //  int (*win_select_menu)(winid, int, MENU_ITEM_P **);
    genl_message_menu, //  char (*win_message_menu)(char, int, const char *);
    sdl2_mark_synch, //  void (*win_mark_synch)(void);
    sdl2_wait_synch, //  void (*win_wait_synch)(void);
#ifdef CLIPPING
    sdl2_cliparound,
#endif
#ifdef POSITIONBAR
...
#endif
    sdl2_print_glyph, // void (*win_print_glyph)(winid, coordxy, coordxy, const glyph_info *, const glyph_info *);
    sdl2_raw_print, // void (*win_raw_print)(const char *);
    sdl2_raw_print_bold, // void (*win_raw_print_bold)(const char *);
    sdl2_nhgetch, // int (*win_nhgetch)(void);
    sdl2_nh_poskey, // int (*win_nh_poskey)(coordxy *, coordxy *, int *);
    sdl2_nhbell, // void (*win_nhbell)(void);
    sdl2_doprev_message, // int (*win_doprev_message)(void);
    sdl2_yn_function, // char (*win_yn_function)(const char *, const char *, char);
    sdl2_getlin, // void (*win_getlin)(const char *, char *);
    sdl2_get_ext_cmd, // int (*win_get_ext_cmd)(void);
    sdl2_number_pad, // void (*win_number_pad)(int);
    sdl2_delay_output, // void (*win_delay_output)(void);
#ifdef CHANGE_COLOR
    sdl2_change_color, // void (*win_change_color)(int, long, int);
    sdl2_get_color_string, // char *(*win_get_color_string)(void);
#endif
    genl_outrip, // void (*win_outrip)(winid, int, time_t);
    sdl2_preference_update, // void (*win_preference_update)(const char *);
    sdl2_getmsghistory, // char *(*win_getmsghistory)(boolean);
    sdl2_putmsghistory, // void (*win_putmsghistory)(const char *, boolean);
    sdl2_status_init, // void (*win_status_init)(void);
    sdl2_status_finish, // void (*win_status_finish)(void);
    sdl2_status_enablefield, // void (*win_status_enablefield)(int, const char *, const char *, boolean);
    sdl2_status_update, // void (*win_status_update)(int, genericptr_t, int, int, int, unsigned long *);
    genl_can_suspend_no, // boolean (*win_can_suspend)(void);
    sdl2_update_inventory, // void (*win_update_inventory)(int);
    sdl2_ctrl_nhwindow, // win_request_info *(*win_ctrl_nhwindow)(winid, int, win_request_info *);
};

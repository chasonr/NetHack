...
select_role(void)
{
    ...
    for (i = 0; roles[i].name.m != NULL; ++i) {
        if (valid_selection(i, flags.initrace, flags.initgend, flags.initalign)) {
            ...
            ++count;
            id.a_int = i + 1;
            sdl2_menu_addMenu(menu, NULL, &id, 0, 0, 0, 0, text, MENU_ITEMFLAGS_NONE);
        }
    }
    ...
    id.a_int = -1;
    sdl2_menu_addMenu(menu, NULL, &id, '*', 0, 0, 0, "Random", MENU_ITEMFLAGS_NONE);

...
struct Glyph {
    ...
    /* Zap effect */
    glyph_info zap_glyph;

    /* Explosion effect */
    glyph_info expl_glyph;
    clock_t expl_timer;
};
...
static void sdl2_map_printGlyph(struct SDL2Window *win, coordxy x, coordxy y,
...
sdl2_map_redraw(struct SDL2Window *win)
{
    ...
    for (y = 0; y < ROWNO; ++y) {
        for (x = 0; x < COLNO; ++x) {
            if (data->m_map[y][x].zap_glyph.glyph != NO_GLYPH
...
sdl2_map_clear(struct SDL2Window *win)
{
    ...
    blank.glyph = background_glyph;
    blank.ttychar = ' ';
    blank.framecolor = 0;
    blank.gm = glyphmap[background_glyph];
    for (y = 0; y < ROWNO; ++y) {
        for (x = 0; x < COLNO; ++x) {
            data->m_map[y][x].bkglyph = blank;
            data->m_map[y][x].glyph = blank;
            data->m_map[y][x].zap_glyph.glyph = NO_GLYPH;
            data->m_map[y][x].expl_glyph.glyph = NO_GLYPH;
            data->m_map[y][x].expl_timer = 0;
        }
    }
...
static void
sdl2_map_printGlyph(struct SDL2Window *win, coordxy x, coordxy y,
        const glyph_info *glyph, const glyph_info *bkglyph)
{
    struct MapPrivate *data = (struct MapPrivate *) win->data;
    ...
    if (GLYPH_EXPLODE_OFF <= glyph->glyph
    &&  glyph->glyph <= GLYPH_EXPLODE_OFF + MAXEXPCHARS * EXPL_MAX) {
        data->m_map[y][x].expl_glyph = *glyph;
        data->m_map[y][x].expl_timer = clock() + 1 * CLOCKS_PER_SEC;
    } else if (GLYPH_ZAP_OFF <= glyph->glyph
    &&  glyph->glyph <= GLYPH_ZAP_OFF + 4 * NUM_ZAP) {
...
sdl2_map_draw(struct SDL2Window *win, int x, int y, boolean cursor)
{
    ...
    SDL_Rect tile, cell;
    const glyph_info *glyphs[5]; /* background, map, zap, explosion, shield */
    unsigned i;
    Uint32 bg;
    ...
    if (data->m_tile_mode) {
        ...
        for (i = 0; i < SIZE(glyphs); ++i) {
            if (glyphs[i] != NULL && glyphs[i]->glyph != NO_GLYPH) {
                tilenum = glyphs[i]->gm.tileidx;
        ...
        for (i = 0; i < SIZE(glyphs); ++i) {
            if (glyphs[i] != NULL) {
                ...
                if (SYMHANDLING(H_UTF8) && glyphs[i]->gm.u && glyphs[i]->gm.u->utf8str) {
                    ochar = glyphs[i]->gm.u->utf32ch;
                } else
#endif
                {
                    ch = glyphs[i]->ttychar;
                ...
                if (glyphs[i]->gm.customcolor != 0) {
                    ocolor = glyphs[i]->gm.customcolor;
                    text_fg.r = (Uint8)(ocolor >> 16);
                    text_fg.g = (Uint8)(ocolor >>  8);
                    text_fg.b = (Uint8)(ocolor >>  0);
                    text_fg.a = 255;
                } else {
                    ocolor = glyphs[i]->gm.sym.color;
...
/*ARGSUSED*/
void
sdl2_map_update_cursor(struct SDL2Window *window, int index,
                       genericptr_t ptr UNUSED, int chg UNUSED,
                       int percent UNUSED, int color,
                       const unsigned long *colormasks UNUSED)
{

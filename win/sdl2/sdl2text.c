...
sdl2_text_redraw(struct SDL2Window *win)
{
    ...
    foreground = sdl2_text_fg(iflags.wcolors[wcolor_text].fg,
                              iflags.wcolors[wcolor_text].bg,
                              ATR_NONE);
    background = sdl2_text_bg(iflags.wcolors[wcolor_text].fg,
                              iflags.wcolors[wcolor_text].bg,
                              ATR_NONE, 160);
    rect.x = 0;
    rect.y = 0;
    ...
    for (i = 0; i < data->m_page_size; ++i) {
        ...
        sdl2_window_renderStrBG(win, data->m_contents[j].text, margin, y,
                                sdl2_text_fg(iflags.wcolors[wcolor_text].fg,
                                             iflags.wcolors[wcolor_text].bg,
                                             attr),
                                sdl2_text_bg(iflags.wcolors[wcolor_text].fg,
                                             iflags.wcolors[wcolor_text].bg,
                                             attr, 0));
        y += win->m_line_height;
    }

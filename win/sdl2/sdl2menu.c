...
static inline boolean selected(const struct MenuEntry *entry)
{
    return (entry->itemflags & MENU_ITEMFLAGS_SELECTED) != 0;
}
...
sdl2_menu_redraw(struct SDL2Window *win)
{
    ...
    foreground = sdl2_text_fg(iflags.wcolors[wcolor_menu].fg,
                              iflags.wcolors[wcolor_menu].bg,
                              ATR_NONE);
    background = sdl2_text_bg(iflags.wcolors[wcolor_menu].fg,
                              iflags.wcolors[wcolor_menu].bg,
                              ATR_NONE, 160);
    rect.x = 0;
    rect.y = 0;
    ...
    for (i = 0; i < data->m_page_size; ++i) {
        ...
        if (data->m_menu[j].color < 0) {
            color = iflags.wcolors[wcolor_menu].fg;
        } else {
        ...
        if (data->m_menu[j].identifier.a_void == NULL && num_columns == 1) {
            ...
            if (data->m_menu[j].identifier.a_void != NULL) {
                ...
                snprintf(tag, SIZE(tag), "%c %c - ",
                        !selected(&data->m_menu[j]) ? ' ' : data->m_menu[j].count == 0 ? '*' : '#',
                        ch);
...
void
sdl2_menu_startMenu(struct SDL2Window *win, unsigned long mbehavior UNUSED)
{
    struct MenuPrivate *data = (struct MenuPrivate *) win->data;
...
void
sdl2_menu_addMenu(struct SDL2Window *win, const glyph_info *glyph,
    ...
    if (how == PICK_ANY) {
        ...
        case MENU_SELECT_ALL:
            ...
            for (i = 0; i < data->m_menu_size; ++i) {
                ...
                data->m_menu[i].itemflags |= MENU_ITEMFLAGS_SELECTED;
